...
RSpec.describe PubSubModelSync::ServiceKafka do
  ...
  # Stands in for a ruby-kafka message: the service only reads these two.
  let(:message_klass) { Struct.new(:value, :headers, keyword_init: true) }
  let(:message) do
    message_klass.new(value: payload.to_json,
                      headers: { 'service_model_sync' => true })
  end
  let(:invalid_message) do
    message_klass.new(headers: { 'invalid_partition' => true })
  end
  ...
  describe 'when listening fails' do
    before do
      allow(service).to receive(:consumer).and_return(consumer)
      allow(inst).to receive(:log)
    end

    it 'logs a clean stop when the runner shuts it down' do
      allow(consumer).to receive(:each_message).and_raise(PubSubModelSync::Runner::ShutDown)
    ...
    it 'logs any other error' do
      allow(consumer).to receive(:each_message).and_raise('Invalid message')
