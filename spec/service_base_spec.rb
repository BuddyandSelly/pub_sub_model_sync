...
RSpec.describe PubSubModelSync::ServiceBase do
  ...
  describe 'when the service does not implement the interface' do
    it 'names the missing method' do
      expect { inst.listen_messages }.to raise_error(NoMethodError, /listen_messages/)
      expect { inst.publish(payload) }.to raise_error(NoMethodError, /publish/)
      expect { inst.stop }.to raise_error(NoMethodError, /stop/)
    end
  end

  it 'parses the payload without logging it when not debugging' do
