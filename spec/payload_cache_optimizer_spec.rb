...
RSpec.describe PubSubModelSync::PayloadCacheOptimizer do
  ...
  it 'optimizes the payload without logging it when not debugging' do
    allow(PubSubModelSync::Config).to receive(:debug).and_return(false)
    stub_cached_payload(data.merge(title: 'old title'))
