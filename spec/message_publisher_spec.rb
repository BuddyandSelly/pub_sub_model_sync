...
RSpec.describe PubSubModelSync::MessagePublisher do
  ...
  describe 'when a transaction fails' do
    it 'rolls it back and re-raises' do
      expect { described_class.transaction('key') { raise('Invalid data') } }
        .to raise_error('Invalid data')
      expect(described_class.current_transaction).to be_nil
    end
  end

  # forced_ordering_key: true means "keep my own ordering_key", even when the
  # transaction the payload is published in defines one of its own.
  describe 'when the payload forces its own ordering_key' do
    let(:headers) { { ordering_key: 'own_key', forced_ordering_key: true } }
    let(:payload) do
      payload_klass.new({}, { klass: publisher_klass, action: :greeting }, headers)
    end

    it 'ignores the key of the transaction' do
      described_class.transaction('transaction_key') { payload.publish! }

      expect(payload.headers[:ordering_key]).to eq('own_key')
    end
  end

  describe 'when not debugging' do
    before { allow(inst.config).to receive(:debug).and_return(false) }
    ...
    it 'publishes model notifications' do
      expect(connector).to receive(:publish)
