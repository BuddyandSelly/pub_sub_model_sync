...
RSpec.describe PubSubModelSync::MessageProcessor do
  ...
  describe 'when not debugging' do
    ...
    it 'processes the message without logging each step' do
      stub_with_subscriber(action) do
        expect(s_processor).to receive(:call)
        inst.process
      end
    end
  end

  it 'does not log the processed message when :on_success_processing asks for it' do
    allow(inst.config.on_success_processing).to receive(:call).and_return(:skip_log)
    stub_with_subscriber(action) do
      expect(inst).not_to receive(:log).with(include('processed message with'))
      inst.process
    end
  end
end
